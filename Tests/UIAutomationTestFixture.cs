...
namespace ODDGames.Bugpunch
{
    ...
    public abstract class UIAutomationTestFixture
    {
        ...
        public virtual async Task TearDown()
        {
            ...
            // Read NUnit's actual outcome and feed it to TestReport before EndSession finalizes.
            try
            {
                var status = TestContext.CurrentContext.Result.Outcome.Status;
                TestReport.SetOutcome(status == NUnit.Framework.Interfaces.TestStatus.Passed);
            }
            catch { }
