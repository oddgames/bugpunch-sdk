...
namespace ODDGames.UIAutomation.AI
{
    ...
    public class AITestRunner
    {
        ...
        /// <summary>
        /// Initialize cleanup handlers at runtime startup.
        /// </summary>
        ...
        private string GetActionTarget(AIAction action)
        {
            return action switch
            {
                ...
                DoubleClickAction doubleClick => doubleClick.Search?.value ?? $"({doubleClick.ScreenPosition?.x:F2},{doubleClick.ScreenPosition?.y:F2})",
                HoldAction hold => hold.Search?.value,
                TypeAction type => type.Search?.value,
                DragAction drag => drag.FromSearch?.value,
                ScrollAction scroll => scroll.Search?.value,
                SwipeAction swipe => swipe.Search?.value,
                TwoFingerSwipeAction twoFingerSwipe => twoFingerSwipe.Search?.value,
                PinchAction pinch => pinch.Search?.value,
                RotateAction rotate => rotate.Search?.value,
                SetSliderAction setSlider => setSlider.Search?.value,
                SetScrollbarAction setScrollbar => setScrollbar.Search?.value,
