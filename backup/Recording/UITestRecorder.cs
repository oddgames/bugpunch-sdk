...
namespace ODDGames.UIAutomation
{
    public class UITestRecorder : MonoBehaviour
    {
        ...
        public void StartRecording(string testName = null)
        {
            ...
            // Read pending recording config via PlayerPrefs (same keys used by UITestSettings)
            const string pendingFolderKey = "ODDGames.UIAutomation.PendingRecordingFolder";
            const string pendingTestDataKey = "ODDGames.UIAutomation.PendingTestDataSource";

            LastRecordingFolder = PlayerPrefs.GetString(pendingFolderKey, "");
            if (!string.IsNullOrEmpty(LastRecordingFolder))
                PlayerPrefs.DeleteKey(pendingFolderKey);

            testDataSource = PlayerPrefs.GetString(pendingTestDataKey, "");
            if (!string.IsNullOrEmpty(testDataSource))
                PlayerPrefs.DeleteKey(pendingTestDataKey);

            if (string.IsNullOrEmpty(LastRecordingFolder))
            {
                LastRecordingFolder = Path.Combine(Application.dataPath, "UIAutomationTests", "GeneratedTests", baseName);
            }
        ...
        public void StopRecording()
        {
            ...
            // Save last recording folder for UITestSettings
            PlayerPrefs.SetString("ODDGames.UIAutomation.LastRecordingFolder", LastRecordingFolder);
        ...
        public void RecordKeyPress(KeyCode key, string targetName, string targetPath)
        {
            if (!IsRecording) return;

            ShowActionFeedback("KEY", key.ToString(), new Vector2(Screen.width / 2f, Screen.height / 2f), new Color(0.6f, 0.4f, 0.9f));

            Debug.Log($"[UITestRecorder] KEYPRESS recorded - Key: {key} Target: '{targetName}'");

            string screenshotPath = CaptureScreenshot($"step_{stepCounter:D4}_keypress");

            AddStep(new UITestRecordingStep
            {
                type = UITestRecordingStep.StepType.KeyPress,
                timestamp = Time.realtimeSinceStartup - recordingStartTime,
