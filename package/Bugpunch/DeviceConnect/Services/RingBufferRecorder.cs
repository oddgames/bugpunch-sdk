...
namespace ODDGames.Bugpunch.DeviceConnect
{
    ...
    public class RingBufferRecorder : MonoBehaviour
    {
        ...
        {
#if UNITY_EDITOR
            Debug.Log("[Bugpunch] RingBufferRecorder: not available in editor");
            ...
            {
                Debug.LogError($"[Bugpunch] RingBufferRecorder.StartRecording (Android) failed: {e.Message}");
                OnStarted?.Invoke(false);
            }
            ...
            try
            {
                ...
                IsRecording = ok;
                Debug.Log($"[Bugpunch] RingBufferRecorder.StartRecording (iOS): {(ok ? "OK" : "FAILED")}");
                OnStarted?.Invoke(ok);
            }
            catch (Exception e)
            {
                Debug.LogError($"[Bugpunch] RingBufferRecorder.StartRecording (iOS) failed: {e.Message}");
                OnStarted?.Invoke(false);
            }

#else
            Debug.Log($"[Bugpunch] RingBufferRecorder: platform {Application.platform} not supported");
            OnStarted?.Invoke(false);
#endif
        }
        ...
        public void StopRecording()
        {
            ...
            {
                Debug.LogError($"[Bugpunch] RingBufferRecorder.StopRecording (Android) failed: {e.Message}");
            }
            ...
            {
                Debug.LogError($"[Bugpunch] RingBufferRecorder.StopRecording (iOS) failed: {e.Message}");
            }
#endif
            IsRecording = false;
        }
        ...
        public bool DumpToFile(string outputPath)
        {
            ...
            {
                Debug.LogError($"[Bugpunch] RingBufferRecorder.DumpToFile (Android) failed: {e.Message}");
                return false;
            }
            ...
            {
                Debug.LogError($"[Bugpunch] RingBufferRecorder.DumpToFile (iOS) failed: {e.Message}");
                return false;
            }
        ...
        public byte[] DumpToBytes()
        {
            ...
            {
                Debug.LogError($"[Bugpunch] RingBufferRecorder.DumpToBytes read failed: {e.Message}");
                return null;
            }
        }
