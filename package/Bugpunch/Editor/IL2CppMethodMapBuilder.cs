...
namespace ODDGames.Bugpunch.Editor
{
    ...
    public static class IL2CppMethodMapBuilder
    {
        ...
        public static string BuildAndStage(string projectRoot)
        {
            ...
            if (map.Count == 0)
            {
                Debug.Log($"[Bugpunch] IL2CPP method map: scanned {filesScanned} cpp files but found 0 source_info markers. " +
            ...
            {
                Debug.LogWarning($"[Bugpunch] Failed to stage IL2CPP method map: {ex.Message}");
                try { if (File.Exists(outPath)) File.Delete(outPath); } catch { }
                return null;
            }

            var sizeKb = new FileInfo(outPath).Length / 1024.0;
            Debug.Log($"[Bugpunch] IL2CPP method map: {map.Count} methods from {filesScanned} cpp files, {sizeKb:F1} KB gzipped.");
            return outPath;
        }
        ...
        static void ScanFile(string path, Dictionary<string, (string file, int line)> map)
        {
            ...
            {
                Debug.LogWarning($"[Bugpunch] IL2CPP cpp scan failed for {Path.GetFileName(path)}: {ex.Message}");
            }
        }
