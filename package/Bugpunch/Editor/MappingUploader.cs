    ...
    public class BugpunchMappingUploader : IPostprocessBuildWithReport
    {
        ...
        public void OnPostprocessBuild(BuildReport report)
        {
            ...
            if (Application.isBatchMode)
            {
                ...
                catch (Exception ex) { Debug.LogError($"[Bugpunch] Mapping upload failed: {ex.Message}"); }
                return;
            }
            ...
            bool interactive)
        {
            ...
            try
            {
                ...
                if (size <= 0)
                {
                    Debug.LogWarning($"[Bugpunch] mapping.txt at {mappingPath} is empty — skipping.");
                    return;
                }
                if (size > MaxMappingBytes)
                {
                    Debug.LogWarning($"[Bugpunch] mapping.txt is {size / 1048576.0:F1} MB — exceeds server limit; skipping.");
                    return;
                }
            ...
            string mappingPath, string bundleId, string version, string buildCode, string gitSha)
        {
            ...
            try
            {
                ...
                {
                    Debug.Log($"[Bugpunch] mapping.txt uploaded for {bundleId} v{version} ({buildCode}).");
                }
                else
                {
                    Debug.LogWarning($"[Bugpunch] mapping.txt upload failed: {req.error} — {req.downloadHandler?.text}");
                }
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[Bugpunch] mapping.txt upload threw: {ex.Message}");
            }
            finally
            {
            ...
            string mappingPath, string bundleId, string version, string buildCode, string gitSha)
        {
            ...
            {
                Debug.LogWarning($"[Bugpunch] Failed to stage mapping upload body: {ex.Message}");
                try { if (File.Exists(bodyPath)) File.Delete(bodyPath); } catch { }
                return null;
            }
        }
