...
namespace ODDGames.Bugpunch.Editor
{
    ...
    public class BugpunchSymbolUploader : IPostprocessBuildWithReport
    {
        ...
        public void OnPostprocessBuild(BuildReport report)
        {
            ...
            {
                Debug.LogWarning($"[Bugpunch] Invalid serverUrl '{config.serverUrl}' — skipping symbol upload.");
                return;
            }
            ...
            {
                Debug.Log($"[Bugpunch] Android Symbols level is '{symLevel}' — skipping upload. " +
            ...
            {
                Debug.Log("[Bugpunch] Android Symbols level is 'Debugging' (Full DWARF) — uploading. " +
            ...
            CancellationToken ct, ExtractState state)
        {
            ...
            foreach (var entry in archive.Entries)
            {
                ...
                if (string.IsNullOrEmpty(buildId))
                {
                    ...
                    Debug.Log($"[Bugpunch] {entry.FullName} has no GNU build-id — can't be symbolicated, skipping. " +
            ...
            CancellationTokenSource cts)
        {
            ...
            {
                Debug.LogWarning("[Bugpunch] Symbol check cancelled.");
                return;
            }
            ...
            var summary = $"[Bugpunch] Uploaded {uploaded}/{missing.Count} missing symbol files " +
            ...
            bool interactive, CancellationTokenSource cts)
        {
            ...
            {
                Debug.LogWarning($"[Bugpunch] IL2CPP method map build failed: {ex.Message}");
                return;
            }
            ...
            try
            {
                ...
                Debug.Log($"[Bugpunch] IL2CPP method map uploaded for {mUploaded}/{il2cppBuildIds.Count} libil2cpp build-id(s)" +
            ...
            CancellationTokenSource cts)
        {
            ...
            try
            {
                ...
                Debug.LogWarning($"[Bugpunch] IL2CPP map upload failed for {buildId}: " +
                    $"{req.error} — {req.downloadHandler?.text}");
                return false;
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[Bugpunch] IL2CPP map upload threw for {buildId}: {ex.Message}");
                return false;
            }
            finally
            {
        ...
        static string StageMappingMultipart(out string boundary, string buildId, string filePath)
        {
            ...
            {
                Debug.LogError($"[Bugpunch] Failed to stage IL2CPP mapping body: {ex.Message}");
                try { if (File.Exists(bodyPath)) File.Delete(bodyPath); } catch { }
                return null;
            }
        }
            ...
            CancellationTokenSource cts)
        {
            ...
            {
                Debug.LogError($"[Bugpunch] Symbol temp file missing before upload: {f.TempPath}");
            ...
            {
                Debug.LogError($"[Bugpunch] SHA-256 failed for {f.Filename}: {ex.Message}");
                return UploadResult.Failed;
            }
            ...
            for (int attempt = 1; attempt <= MaxUploadAttempts; attempt++)
            {
                ...
                Debug.LogWarning($"[Bugpunch] Direct upload attempt {attempt}/{MaxUploadAttempts} failed for {f.Filename}; " +
            ...
            bool interactive, CancellationTokenSource cts)
        {
            ...
            try
            {
                ...
                {
                    Debug.LogError($"[Bugpunch] /upload-direct/init failed for {f.Filename}: {ex.Message}");
                ...
                {
                    Debug.LogError($"[Bugpunch] {f.Filename}: {failedParts.Count} of {init.Parts.Count} parts failed to upload to S3.");
                    return (UploadResult.Failed, true);
                }
                ...
                {
                    Debug.LogError($"[Bugpunch] /upload-direct/complete failed for {f.Filename}: {ex.Message}");
                    return (UploadResult.Failed, true);
                }
            }
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[Bugpunch] Direct upload threw for {f.Filename}: {ex.Message}");
                return (UploadResult.Failed, true);
            }
            finally
            {
            ...
            CancellationTokenSource cts)
        {
            ...
            {
                Debug.LogWarning($"[Bugpunch] failed to read part bytes: {ex.Message}");
                return null;
            }
            ...
            {
                if (cts.IsCancellationRequested) return null;
                Debug.LogWarning($"[Bugpunch] S3 part timed out after {S3PartTimeoutSeconds:F0}s.");
                return null;
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[Bugpunch] S3 PUT threw: {ex.Message}");
                return null;
            }
        }
            ...
            CancellationTokenSource cts)
        {
            ...
            Debug.LogError($"[Bugpunch] /complete HTTP {(int)resp.StatusCode} — {text}");
            return false;
        }
            ...
            string baseUrl, string apiKey, InitResponse init, CancellationTokenSource cts)
        {
            ...
            {
                Debug.LogWarning($"[Bugpunch] /abort failed (non-fatal): {ex.Message}");
            }
        }
