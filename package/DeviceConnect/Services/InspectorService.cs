...
namespace ODDGames.Bugpunch.DeviceConnect
{
    public class InspectorService
    {
        ...
        /// <paramref name="debug"/> == false → editor-like view (filtered via InspectorSchema).
        /// <paramref name="debug"/> == true  → raw reflection view (matches Unity's Inspector
        /// "Debug" mode — everything, including internals and statics).
        /// </summary>
        public string GetFields(string instanceIdStr, string componentIdStr, bool debug = false)
        {
            ...
            foreach (var p in type.GetProperties(flags))
            {
                ...
                if (!debug && !InspectorSchema.ShouldShow(type, p, isSerializedField: false)) continue;
            ...
            {
                var serialized = InspectorSchema.IsUnitySerialized(f);
                if (!debug)
                {
                    if (!InspectorSchema.ShouldShow(type, f, serialized)) continue;
                }
                else if (!f.IsPublic && f.GetCustomAttribute<SerializeField>() == null && !serialized)
                {
                    // In debug mode we still skip compiler-generated backing fields
                    // (<X>k__BackingField) since they duplicate the property view.
