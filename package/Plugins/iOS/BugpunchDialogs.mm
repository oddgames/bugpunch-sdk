...
- (void)viewDidLoad {
    ...
    // Tap-to-copy device ID footer — gives QA a visible identifier on every
    // Bugpunch dialog so they can read it out without digging into settings.
    NSString* bpStableId = [NSString stringWithUTF8String:Bugpunch_GetStableDeviceId() ?: ""];
    UILabel* idLabel = [[UILabel alloc] init];
    idLabel.text = [NSString stringWithFormat:@"ID: %@", bpStableId];
    idLabel.font = [UIFont fontWithName:@"Menlo" size:10] ?: [UIFont systemFontOfSize:10];
    idLabel.textColor = [UIColor colorWithWhite:0.48 alpha:1];
    idLabel.textAlignment = NSTextAlignmentCenter;
    idLabel.userInteractionEnabled = YES;
    idLabel.translatesAutoresizingMaskIntoConstraints = NO;
    idLabel.accessibilityLabel = [NSString stringWithFormat:@"Device ID %@", bpStableId];
    UITapGestureRecognizer* idTap = [[UITapGestureRecognizer alloc]
        initWithTarget:self action:@selector(onDeviceIdTap:)];
    [idLabel addGestureRecognizer:idTap];
    objc_setAssociatedObject(idLabel, "bpStableId", bpStableId, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        // Device ID footer
        [idLabel.topAnchor constraintEqualToAnchor:submitBtn.bottomAnchor constant:12],
        [idLabel.leadingAnchor constraintEqualToAnchor:content.leadingAnchor constant:pad],
        [idLabel.trailingAnchor constraintEqualToAnchor:content.trailingAnchor constant:-pad],

        // Bottom anchor for scroll content sizing
        [idLabel.bottomAnchor constraintEqualToAnchor:content.bottomAnchor constant:-pad],
    ]];
}

- (void)onDeviceIdTap:(UITapGestureRecognizer*)g {
    NSString* id_ = objc_getAssociatedObject(g.view, "bpStableId");
    if (id_.length == 0) return;
    [UIPasteboard generalPasteboard].string = id_;
    // Flash label to acknowledge the copy.
    UILabel* label = (UILabel*)g.view;
    NSString* original = label.text;
    label.text = @"Copied ✓";
