...
- (UIStackView*)buildFormColumnIncludingHeader:(BOOL)includeHeader {
    ...
    [form addArrangedSubview:[self buildDeviceIdFooter]];

    return form;
}

// Tap-to-copy device ID — every Bugpunch native surface carries it so QA can
// read the identifier out of any dialog without digging into settings.
- (UILabel*)buildDeviceIdFooter {
    NSString* bpStableId = [NSString stringWithUTF8String:Bugpunch_GetStableDeviceId() ?: ""];
    UILabel* l = [UILabel new];
    l.text = [NSString stringWithFormat:@"ID: %@", bpStableId];
    l.font = [UIFont fontWithName:@"Menlo" size:10] ?: [UIFont systemFontOfSize:10];
    l.textColor = BP_COLOR_TEXT_LABEL;
    l.textAlignment = NSTextAlignmentCenter;
    l.userInteractionEnabled = YES;
    l.accessibilityLabel = [NSString stringWithFormat:@"Device ID %@", bpStableId];
    objc_setAssociatedObject(l, "bpStableId", bpStableId, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
