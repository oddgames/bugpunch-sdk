...
namespace ODDGames.UIAutomation.AI
{
    ...
    public class AITestSettings : ScriptableObject
    {
        ...
        /// In Editor: loads from Assets/Editor/AITestSettings.asset (creates if missing).
        /// At Runtime: must be set via SetInstance() or loaded from Resources.
        /// </summary>
        public static AITestSettings Instance
        {
            get
            {
                ...
                {
#if UNITY_EDITOR
                    instance = LoadOrCreateEditor();
#else
                    // At runtime, try to load from Resources
                    instance = Resources.Load<AITestSettings>("AITestSettings");
                    if (instance == null)
                    {
                        Debug.LogWarning("[AITestSettings] No instance found. Call AITestSettings.SetInstance() or place AITestSettings in Resources folder.");
                    }
#endif
                }
                return instance;
        ...
        private static AITestSettings LoadOrCreateEditor()
        {
