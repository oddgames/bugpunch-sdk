...
namespace ODDGames.UIAutomation.DeviceConnect
{
    public class TunnelClient
    {
        ...
        public TunnelClient(OddDevConfig config)
        {
            _config = config;
        }
        ...
        public async void Connect()
        {
            ...
            try
            {
                ...
                Debug.Log($"[OddDev] Connecting to {uri}...");
        ...
        public void ProcessMessages()
        {
            ...
            {
                try { action(); }
                catch (Exception ex) { Debug.LogError($"[OddDev] Queue error: {ex}"); }
            }
        }
        ...
        async Task SendAsync(string message)
        {
            ...
            {
                Debug.LogError($"[OddDev] Send error: {ex.Message}");
            }
        }
        ...
        void HandleMessage(string json)
        {
            try
            {
                ...
                else if (json.Contains("\"type\":\"registered\"") || json.Contains("\"type\": \"registered\""))
                {
                    ...
                    Debug.Log("[OddDev] Device registered with server");
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"[OddDev] Message parse error: {ex.Message}\n{json}");
            }
        }
