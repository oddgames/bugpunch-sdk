...
namespace ODDGames.UIAutomation.DeviceConnect
{
    ...
    /// and sends to the OddDev server. Triggered by shake gesture, key combo, or API.
    /// </summary>
    public class BugReporter : MonoBehaviour
    {
        ...
        IEnumerator SendReport(string title, string description, BugReportType type)
        {
            ...
            var client = OddDevClient.Instance;
            if (client != null && client.IsConnected)
            {
                // Send as a special tunnel message
                client.Tunnel.SendResponse("bugreport", 200, report.ToString(), "application/json");
                Debug.Log($"[OddDev] Bug report sent ({videoFrames.Count} video frames, {screenshot.Length / 1024}KB screenshot)");
                OnReportSent?.Invoke(true);
            }
            else
            {
                // Queue for later or send via HTTP
                Debug.LogWarning("[OddDev] Not connected — bug report queued");
        ...
        IEnumerator SendFeedbackCoroutine(string message, FeedbackRating rating)
        {
            ...
            if (client != null && client.IsConnected)
            {
                ...
                Debug.Log("[OddDev] Feedback sent");
            }
            else
            {
                Debug.LogWarning("[OddDev] Not connected — feedback not sent");
            }
        }
