...
namespace ODDGames.UIAutomation.DeviceConnect
{
    ...
    public class WebRTCStreamer : MonoBehaviour
    {
        ...
        /// Called from OddDevClient when a webrtc-* message arrives.
        /// </summary>
        ...
        {
            Debug.Log("[OddDev] WebRTC: received offer, creating answer...");
            ...
            {
                Debug.Log($"[OddDev] WebRTC connection state: {state}");
            ...
            {
                Debug.LogError($"[OddDev] WebRTC: SetRemoteDescription failed: {setRemoteOp.Error.message}");
                yield break;
            }
            ...
            if (answerOp.IsError)
            {
                Debug.LogError($"[OddDev] WebRTC: CreateAnswer failed: {answerOp.Error.message}");
                yield break;
            }
            ...
            {
                Debug.LogError($"[OddDev] WebRTC: SetLocalDescription failed: {setLocalOp.Error.message}");
                yield break;
            }
            ...
            Debug.Log("[OddDev] WebRTC: streaming started");
        }
        ...
        IEnumerator HandleAnswer(string sdpJson)
        {
            ...
            if (op.IsError)
                Debug.LogError($"[OddDev] WebRTC: SetRemoteDescription (answer) failed: {op.Error.message}");
        }
        ...
        public void StopStreaming()
        {
            ...
            CleanupPeerConnection();
            CleanupRenderTexture();
            Debug.Log("[OddDev] WebRTC: streaming stopped");
        }
