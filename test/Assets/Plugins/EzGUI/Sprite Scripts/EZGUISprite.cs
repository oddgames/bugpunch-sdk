...
/// </remarks>
[ExecuteInEditMode]
public class EZGUISprite : SpriteBase
{
	/// <summary>
...
/*
	...
	static public EZGUISprite Create(string name, Vector3 pos)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
		return (EZGUISprite)go.AddComponent(typeof(EZGUISprite));
	}
	...
	/// <returns>Returns a reference to the component.</returns>
	static public EZGUISprite Create(string name, Vector3 pos, Quaternion rotation)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
...
public class SpriteMirror : SpriteRootMirror
{
	...
	public override void Mirror(SpriteRoot s)
	{
		...
		lowerLeftPixel = ((EZGUISprite)s).lowerLeftPixel;
		pixelDimensions = ((EZGUISprite)s).pixelDimensions;
	}
	...
	public override bool DidChange(SpriteRoot s)
	{
		if (base.DidChange(s))
			return true;
		if (((EZGUISprite)s).lowerLeftPixel != lowerLeftPixel)
		{
			s.uvsInitialized = false;
			return true;
		}
		if (((EZGUISprite)s).pixelDimensions != pixelDimensions)
		{
			s.uvsInitialized = false;
			return true;
		}
