...
public class SpriteText : MonoBehaviour, IUseCamera
{
    ...
    public float maxWidth
    {
        ...
        get { return SpriteRoot.scale * _maxWidth; }
    ...
    [HideInInspector]
    public bool
            isClone = false;                // Set this to true when the SpriteText object has been instantiated from another SpriteText in the scene.
    ...
    public string originalFontName = string.Empty;
    ...
    public virtual void Awake()
    {
        ...
        removeUnsupportedCharacters = false;
    ...
    protected bool CreateMesh()
    {
        ...
        m_pop.name = this.gameObject.name;
        m_push.name = this.gameObject.name;

        meshFilter.sharedMesh = m_pop;

        popped = true;

        if (persistent)
        {
            GameObject.DontDestroyOnLoad(m_pop);
            GameObject.DontDestroyOnLoad(m_push);
        }
        ...
        while (strOffset < str.Length)
        {
            ...
            { // Else it's just another character:
              // See if we're word-wrapping:
                ...
                { // Else add the characters between here and the next color tag:
...
*/
    ...
    public void UpdateMesh()
    {
        ...
        if (!popped)
        {
            ...
            m_pop.vertices = vertices;
            m_pop.uv = UVs;
            m_pop.colors = meshColors;
            m_pop.triangles = faces;
#if WANT_NORMALS
            if (vertCountChanged)
                m_pop.RecalculateNormals();
#endif
            m_pop.RecalculateBounds();
        ...
        else
        {
            ...
            m_push.vertices = vertices;
            m_push.uv = UVs;
            m_push.colors = meshColors;
            m_push.triangles = faces;
#if WANT_NORMALS
            if (vertCountChanged)
                m_push.RecalculateNormals();
#endif
            m_push.RecalculateBounds();

            //Destroy(meshFilter.mesh);
            meshFilter.sharedMesh = m_push;
    ...
    protected void Layout_Multiline(string[] lines)
    {
        ...
        float distBetweenLines = lineSpaceSize - (characterSize * SpriteRoot.scale);
        totalHeight = (characterSize * SpriteRoot.scale) * ((float)lines.Length) + distBetweenLines * ((float)lines.Length - 1);
    ...
    public void CalcSize()
    {
        ...
        if (pixelPerfect)
        {
            ...
            characterSize = ((float)spriteFont.PixelSize) * worldUnitsPerScreenPixel * SpriteRoot.scale;

            worldUnitsPerTexel = worldUnitsPerScreenPixel * SpriteRoot.scale;
    ...
    public bool Persistent
    {
        ...
        set
        {
            if (value == true)
            {
                ...
                DontDestroyOnLoad(m_pop);
                DontDestroyOnLoad(m_push);
                persistent = value;
            }
        }
    }
    ...
    public Vector3[] GetVertices()
    {
        if (popped)
        {
            return m_pop.vertices;
        }
        else
        {
            return m_push.vertices;
        }
    }
    ...
    void DrawBounds()
    {
        ...
        else
        {
            ...
            Gizmos.DrawCube(this.meshRenderer.bounds.center + bounds.center, bounds.extents * 2f);

            if (doBounds)
            {
                maxWidthInPixels = true;
                maxWidth = BoundsToScreenRect(bounds).width + ((GetWidth("S") / worldUnitsPerScreenPixel) * 1.5f);
            }
    ...
    public void DoBounds()
    {
        ...
        if (Application.isPlaying && doBounds)
        {

            float varianceX = meshRenderer.bounds.size.x - bounds.size.x;
            float varianceY = meshRenderer.bounds.size.y - bounds.size.y;
            ...
            if (varianceX > 0 || varianceY > 0)
            {
                ...
                {
                    scaleBy = bounds.size.x / meshRenderer.bounds.size.x;
                }
                else
                {
                    scaleBy = bounds.size.y / meshRenderer.bounds.size.y;
                }
