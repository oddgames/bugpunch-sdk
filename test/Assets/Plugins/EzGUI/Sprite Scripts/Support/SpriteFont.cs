...
public class SpriteChar
{
	...
	/// </summary>
#if UNITY_IPHONE && !(UNITY_3_0 || UNITY_3_1 || UNITY_3_2 || UNITY_3_3 || UNITY_3_4 || UNITY_3_5 || UNITY_3_6 || UNITY_3_7 || UNITY_3_8 || UNITY_3_9 || UNITY_4_0 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3 || UNITY_4_4 || UNITY_4_5 || UNITY_4_6 || UNITY_4_7 || UNITY_4_8 || UNITY_4_9 || UNITY_2018 )
...
public class SpriteFont
{
	...
	void HeaderParser(string line)
	{
		...
		pxSize = Mathf.Abs(intParse(vals[index + 1]));

		index = FindFieldOptional("charSpacing", vals, index);
		if (index != -1)
			charSpacing = Mathf.Abs(floatParse(vals[index + 1]));
	}
	...
	void CommonParser(string line)
	{
		...
		lineHeight = intParse(vals[index + 1]);

		index = FindField("base", vals, index);
		baseHeight = intParse(vals[index + 1]);

		index = FindField("scaleW", vals, index);
		texWidth = intParse(vals[index + 1]);

		index = FindField("scaleH", vals, index);
		texHeight = intParse(vals[index + 1]);

		index = FindField("pages", vals, index);

		if (intParse(vals[index + 1]) > 1)
			Debug.LogError("Multiple pages/textures detected for font \"" + face + "\". only one font atlas is supported.");
	}
	...
	void CharCountParser(string line)
	{
		...
		chars = new SpriteChar[intParse(vals[1]) + 1];
	}
	...
	bool CharParser(string line, int charNum)
	{
		...
		chars[charNum].id = intParse(vals[index + 1]);

		index = FindField("x", vals, index);
		x = floatParse(vals[index + 1]) / (float)texWidth;

		index = FindField("y", vals, index);
		y = 1f - floatParse(vals[index + 1]) / (float)texHeight;

		index = FindField("width", vals, index);
		width = floatParse(vals[index + 1]) / (float)texWidth;

		index = FindField("height", vals, index);
		height = floatParse(vals[index + 1]) / (float)texHeight;

		index = FindField("xoffset", vals, index);
		chars[charNum].xOffset = floatParse(vals[index + 1]);

		index = FindField("yoffset", vals, index);
		chars[charNum].yOffset = -floatParse(vals[index + 1]);

		index = FindField("xadvance", vals, index);
		chars[charNum].xAdvance = intParse(vals[index + 1]);
	...
	{
		string[] vals = line.Split(new char[] { '=' });
		kerningsCount = intParse(vals[1]);
	}

	// Parses the kernings
	bool KerningParser(string line)
	{
		if (!line.Contains("kerning"))
			return false;
		...
		//first = intParse(vals[index + 1]);

		index = FindField("second", vals, index);
		second = intParse(vals[index + 1]);

		index = FindField("amount", vals, index);
		//amount = intParse(vals[index + 1]);
