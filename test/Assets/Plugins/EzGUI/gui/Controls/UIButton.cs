...
public class UIButton : AutoSpriteControlBase
{
	...
	public void TriggerInputDelegate(ref POINTER_INFO ptr)
	{
    ...
    [SerializeField]
    private bool dontAllowMultiTouch = false;
    ...
    public override void OnInput(ref POINTER_INFO ptr)
	{
        ...
        if (ptr.evt == POINTER_INFO.INPUT_EVENT.MOVE_OFF || ptr.evt == POINTER_INFO.INPUT_EVENT.RELEASE || 
            ptr.evt == POINTER_INFO.INPUT_EVENT.RELEASE_OFF || ptr.evt == POINTER_INFO.INPUT_EVENT.TAP)
        {
            lastTouchId = -1;
        }

        if (dontAllowMultiTouch)
            if (lastTouchId != -1 && ptr.id != lastTouchId)
                return;
		...
		if (!m_controlIsEnabled || IsHidden() || !this.isActiveAndEnabled)
		{
		...
		switch (ptr.evt)
		{
			...
			case POINTER_INFO.INPUT_EVENT.TAP:
				...
				if (ptr.type != POINTER_INFO.POINTER_TYPE.TOUCHPAD && this != null &&
	...
	public bool doNotRecreateCollider;
	...
	public override void Start()
	{
		...
		if (Application.isPlaying)
		{
			...
			if (GetComponent<Collider>() == null && !doNotRecreateCollider)
				AddCollider();
