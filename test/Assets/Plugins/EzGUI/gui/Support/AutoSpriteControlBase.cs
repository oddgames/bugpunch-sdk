...
/// inspector.
/// </remarks>
public abstract class AutoSpriteControlBase : AutoSpriteBase, IControl, IUIObject, IPackableControl, IPointerClickHandler, IPointerDownHandler, IPointerUpHandler
{
	//---------------------------------------------------
	...
	public virtual void Copy(SpriteRoot s, ControlCopyFlags flags)
	{
		...
		if ((flags & ControlCopyFlags.Appearance) == ControlCopyFlags.Appearance)
		{
			...
			if (c.GetComponent<Collider>() != null)
			{
				...
				{
					thisCollider = (Collider) gameObject.AddComponent(c.GetComponent<Collider>().GetType());
				...
				if (GetComponent<Collider>().GetType() == c.GetComponent<Collider>().GetType())
				{
					if (c.GetComponent<Collider>() is BoxCollider)
					{
						...
						if (GetComponent<Collider>() == null)
							thisCollider = (Collider) gameObject.AddComponent(typeof(BoxCollider));
					...
					{
						if (GetComponent<Collider>() == null)
							thisCollider = (Collider) gameObject.AddComponent(typeof(SphereCollider));
					...
					{
						if (GetComponent<Collider>() == null)
							thisCollider = (Collider) gameObject.AddComponent(typeof(MeshCollider));
					...
					{
						if (GetComponent<Collider>() == null)
							thisCollider = (Collider) gameObject.AddComponent(typeof(CapsuleCollider));
	...
	public virtual void UpdateCollider()
	{
		...
		if (!(thisCollider is BoxCollider) || IsHidden() || m_spriteMesh == null || customCollider)
			return;

		BoxCollider bc = (BoxCollider) thisCollider;
		
		Bounds bounds = this.transform.Bounds(false);
		
		bc.size = bounds.size;
		
		bc.center = this.transform.InverseTransformPoint(bounds.center);
    ...
    protected EZInputDelegate _inputDelegate;
    protected EZInputDelegate inputDelegate
    {
        get { return _inputDelegate; }
        set
        {
            if (_inputDelegate != null && value != null && _inputDelegate.Method != value.Method)
                Debug.LogWarning(string.Format("Input Delegate is being overwritten on {0}. Old: {1} | New: {2}", this.name, _inputDelegate.Method, (value == null ? "null" : value.Method.ToString())), this);

            _inputDelegate = value;
        }
    }

    protected EZInputDelegate overDelegate;
	...
	public virtual void SetOverDelegate(EZInputDelegate del)
	{
		overDelegate = del;
	}
