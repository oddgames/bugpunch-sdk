...
public struct POINTER_INFO
{
	...
	{
		return string.Format("[POINTER_INFO: type={0}, camera={1}, id={2}, actionID={3}, evt={4}, hitInfo={5}, active={6}, devicePos={7}, origPos={8}, inputDelta={9}, isTap={10}, ray={11}, prevRay={12}, rayDepth={13}, targetObj={14}, layerMask={15}, callerIsControl={16}, activeTime={17}]", type, camera, id, actionID, evt, hitInfo, active, devicePos, origPos, inputDelta, isTap, ray, prevRay, rayDepth, targetObj, layerMask, callerIsControl, activeTime);
	}
...
#if UNITY_3_5 || UNITY_3_6 || UNITY_3_7 || UNITY_3_8 || UNITY_3_9 || UNITY_4_0 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3 || UNITY_4_4 || UNITY_4_5 || UNITY_4_6 || UNITY_4_7 || UNITY_4_8 || UNITY_4_9 || UNITY_2018 
...
public class UIManager : MonoBehaviour
{
	...
	*/
	...
	// Misc.:
	protected Dictionary<string,bool> inputLocks = new Dictionary<string, bool>();				// Keeps track of how many objects have locked input.  This is used to prevent input during panel transitions.
	...
	void Awake()
	{
		...
		dragThreshold = (dragThreshold/384) * Screen.height;
		...
		{
#if UNITY_IPHONE || UNITY_ANDROID
#if UNITY_3_4 || UNITY_3_5 || UNITY_3_6 || UNITY_3_7 || UNITY_3_8 || UNITY_3_9 || UNITY_4_0 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3 || UNITY_4_4 || UNITY_4_5 || UNITY_4_6 || UNITY_4_7 || UNITY_4_8 || UNITY_4_9 || UNITY_2018 
...
// 				else
	...
	protected void DoDragUpdate(POINTER_INFO curPtr)
	{
		...
		RaycastHit[] hits = Physics.RaycastAll(curPtr.ray, curPtr.rayDepth, curPtr.layerMask & dragObj.DropMask, QueryTriggerInteraction.Collide);
		...
		if (hits.Length == 0 || (hits.Length == 1 && hits[0].transform == dragObj.transform))
		{
			...
			for (int i = 0; i < uiCameras.Length; ++i)
			{
				...
				hits = Physics.RaycastAll(ptr.ray, ptr.rayDepth, ptr.layerMask & dragObj.DropMask, QueryTriggerInteraction.Collide);
        ...
        if (UnityEngine.EventSystems.EventSystem.current != null)
        {
            if (Input.touchCount > 0)
                for(int x = 0; x < Input.touchCount; x ++)
                    if (UnityEngine.EventSystems.EventSystem.current.IsPointerOverGameObject(Input.GetTouch(x).fingerId))
                        return;
	...
	System.Collections.Generic.Dictionary<int,float> firstPointerTap = new Dictionary<int, float>();

	const float FIRST_POINTER_TAP_DELAY = 0.1f;

	void SendInput(IUIObject target, ref POINTER_INFO ptr)
	{
		...
		try
		{
            ...
            if (ptr.evt == POINTER_INFO.INPUT_EVENT.TAP && ptr.targetObj != null)
                Debug.Log($"[UI] {ptr.evt} {ptr.targetObj.name}");
	...
	protected void DispatchInput()
	{
		...
		for (int i = 0; i < usedPointers.Length; ++i)
            ...
            //if (PlayerActions.AnyButtonDown)
            //{
			...
			for (int j = 0; j < numActivePointers; ++j)
				for (int i = 0; i < uiCameras.Length; ++i)
#if UNITY_4_0 || UNITY_4_1 || UNITY_4_2 || UNITY_4_3 || UNITY_4_4 || UNITY_4_5 || UNITY_4_6 || UNITY_4_7 || UNITY_4_8 || UNITY_4_9 || UNITY_2018 
	...
	protected void DispatchHelper(ref POINTER_INFO curPtr, int camIndex)
	{
		...
		if (curPtr.targetObj != null)
			curPtr.targetObj.OnOver(curPtr);
		...
		if (inputOutsideViewport != OUTSIDE_VIEWPORT.Process_All && curPtr.type != POINTER_INFO.POINTER_TYPE.RAY)
		{
			...
			if (!curPtr.camera.pixelRect.Contains(curPtr.devicePos) && !Screen.fullScreen)
			{
				...
				if (curPtr.active)
				{
					...
					SendInput(curPtr.targetObj, ref tempPtr);

				}
				else
				{
					tempPtr.evt = POINTER_INFO.INPUT_EVENT.MOVE_OFF;
					SendInput(tempPtr.targetObj, ref tempPtr);
				}
			...
			switch (curPtr.evt)
			{
				...
				case POINTER_INFO.INPUT_EVENT.TAP:
						...
						curPtr.evt == POINTER_INFO.INPUT_EVENT.TAP)
					{
						...
						if (Physics.Raycast(curPtr.ray, out hit, curPtr.rayDepth, curPtr.layerMask, QueryTriggerInteraction.Collide))
						{
						...
						else if (curPtr.targetObj != null)
						{
							...
							SendInput(curPtr.targetObj,ref curPtr);
	...
	protected void PollMouse(ref POINTER_INFO curPtr)
	{
		...
		// was movement:
		if (Input.GetMouseButton(0) && curPtr.active)
		{
			// Drag:
		...
		else if (Input.GetMouseButton(0) && !curPtr.active) // Else if it's a new press, it's a press
		{
		...
		else if (!Input.GetMouseButton(0) && curPtr.active) // A release
		{
			curPtr.inputDelta = Input.mousePosition - curPtr.devicePos;
			curPtr.devicePos = Input.mousePosition;
		...
		else if (!Input.GetMouseButton(0) && Input.mousePosition != curPtr.devicePos) // Mouse was moved
		{
			curPtr.evt = POINTER_INFO.INPUT_EVENT.MOVE;
			curPtr.inputDelta = Input.mousePosition - curPtr.devicePos;
			curPtr.devicePos = Input.mousePosition;
	...
	protected void PollTouchpad()
	{
		...
		for(int i=0; i<numActivePointers; ++i)
		{
			...
			pointers[0, id].ray = uiCameras[0].camera.ScreenPointToRay(ValidateDevicePos(pointers[0, id].devicePos));
		}
		...
		for (int i = 1; i < uiCameras.Length; ++i)
			for (int j = 0; j < numActivePointers; ++j)
			{
				...
				pointers[i, ptrIdx].ray = uiCameras[i].camera.ScreenPointToRay(ValidateDevicePos(pointers[i, ptrIdx].devicePos));
			}

	}

	Vector3 ValidateDevicePos(Vector3 pos)
    {
	...
	protected void PollKeyboard()
	{
		...
		if(!Application.isEditor)
		{
			...
			if(iKeyboard.status == TouchScreenKeyboard.Status.Done || !iKeyboard.active)
			{
		...
		for (int i = 0; i < uiCameras.Length; ++i)
...
#endif
			{
				if (pointers[i, pointerID].targetObj != null)
				{
					...
					SendInput(pointers[i, pointerID].targetObj, ref ptr);
	...
	/// </summary>
	public void LockInput(string key = "default")
	{
		blockInput = true;
        inputLocks[key] = true;
	}
	...
	/// to resume.
	/// </summary>
	public void UnlockInput(string key = "default")
	{
		inputLocks[key] = false;

        blockInput = false;

        foreach (var kvp in inputLocks)
            if (kvp.Value)
                blockInput = true;
